...
export default async function DashboardPage({ searchParams }: PageProps) {
  ...
  return <HomeDashboard month={format(monthStart, "MMMM yyyy", { locale: es })} monthKey={format(monthStart, "yyyy-MM")} availableMonths={availableMonths} totalAmount={totalAmount} members={members} fixedExpenses={bills.filter(b => !b.billType.isCreditCard).map(b => ({ id: b.id, label: b.label, amount: Number(b.amount), billTypeName: b.billType.name, billTypeColor: b.billType.color || "#6b7280", billTypeIcon: b.billType.icon || null }))} creditCardGroups={creditCardGroups} />
}
