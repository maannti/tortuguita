...
// Optional payload field. Validates: each entry has a non-empty org id and a
// percentage in (0, 100], up to 10 entries. The "must total 100%" check
// happens at runtime below since it depends on the array length.
const multiHomeDistributionSchema = z
  .array(
    z.object({
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Multi-home distribution is an optional, nested payload — validated with
    // Zod instead of an unchecked cast (B5).
    const multiHomeDistribution = multiHomeDistributionSchema.parse(
      body.multiHomeDistribution
    )
